...
public class ViewEnrolledStudentsServlet extends HttpServlet {
	...
	private List<EnrolledStudentInfo> getEnrolledStudents(int courseId) {
		...
		String query = "SELECT u.user_id, u.name, u.email, e.enrolled_at, "
				+ "NVL(p.completed_percentage, 0) as progress_percentage " + "FROM Users u "
		...
		try {
			...
			while (rs.next()) {
				...
				EnrolledStudentInfo student = new EnrolledStudentInfo(studentId, username, email, enrollmentDate,
						progressPercentage);
	...
	// If progress needs to be calculated based on material completion instead of
	// using the Progress table
	private int calculateMaterialProgress(Connection conn, int studentId, int courseId) throws SQLException {
		String query = "SELECT " + "COUNT(cm.material_id) as total_materials, "
				+ "COUNT(mp.progress_id) as completed_materials " + "FROM Course_Materials cm "
				+ "LEFT JOIN MaterialProgress mp ON cm.material_id = mp.material_id "
				+ "AND mp.student_id = ? AND mp.is_completed = 1 " + "WHERE cm.course_id = ?";
